...
#if AP_DDS_CAMERA_FEEDBACK_PUB_ENABLED
bool AP_DDS_Client::update_topic(ardupilot_msgs_msg_CameraFeedback& msg, uint8_t camera_instance)
{
    // update_topic(msg.header.stamp);
    // STRCPY(msg.header.frame_id, BASE_LINK_FRAME_ID);

    // Get current vehicle state from AHRS and GPS
    auto &ahrs = AP::ahrs();
    WITH_SEMAPHORE(ahrs.get_semaphore());
    ...
    // Set camera ID from instance
    msg.camera_id = camera_instance;

    // Get camera system information for specific instance
#if AP_CAMERA_ENABLED
    auto *camera = AP_Camera::get_singleton();
    if (camera != nullptr) {
        msg.image_index = camera->image_current_seq(camera_instance);
    } else {
        msg.image_index = 0;
    }
#else
    ...
    if (ahrs.get_location(current_loc)) {
        msg.latitude = current_loc.lat * 1e-7;  // Convert from degE7 to degrees
        msg.longitude = current_loc.lng * 1e-7; // Convert from degE7 to degrees

        // Get altitude
        int32_t altitude_cm;
        if (current_loc.get_alt_cm(Location::AltFrame::ABSOLUTE, altitude_cm)) {
            msg.altitude_msl = altitude_cm * 1e-2f; // Convert cm to meters
        }
        if (current_loc.get_alt_cm(Location::AltFrame::ABOVE_HOME, altitude_cm)) {
            msg.altitude_rel = altitude_cm * 1e-2f; // Convert cm to meters
        }
    }
    ...
    msg.focal_length_mm = 0.0f; // Default value, should come from camera system
    msg.flags = 0; // Default flags
    msg.completed_captures = 0; // Default value, should come from camera system

    // Check if message has changed (simple change detection)
...
bool AP_DDS_Client::update_topic(ardupilot_msgs_msg_CameraCaptureStatus& msg, uint8_t camera_instance)
{
    // Set timestamp
    ...
    auto *camera = AP_Camera::get_singleton();
    if (camera != nullptr) {
        // Get real-time data from the specific camera instance
        uint32_t current_image_count = camera->image_current_seq(camera_instance);
        uint32_t remaining_images = camera->total_image_cap(camera_instance) - current_image_count;

        // Set image status based on real camera state
        if (remaining_images > 0) {
            msg.image_status = 3; // interval set and capture in progress
        } else {
            msg.image_status = 0; // idle
        }

        // Set video status (simplified - would need camera backend support)
        msg.video_status = 0; // idle for now

        // Set image interval (from camera parameters)
        msg.image_interval = 0.0f; // Default, should come from camera parameters

        // Set recording time (not supported yet)
        msg.recording_time_ms = 0;

        // Set available capacity (not supported yet)
        msg.available_capacity = 0.0f;

        // Set image count from actual camera backend
        msg.image_count = current_image_count;
    } else {
        // No camera system available
        msg.image_status = 0;
        msg.video_status = 0;
        msg.image_interval = 0.0f;
        msg.recording_time_ms = 0;
        msg.available_capacity = 0.0f;
        msg.image_count = 0;
    }
...
void AP_DDS_Client::on_request(uxrSession* uxr_session, uxrObjectId object_id, uint16_t request_id, SampleIdentity* sample_id, ucdrBuffer* ub, uint16_t length)
{
    ...
    case services[to_underlying(ServiceIndex::IMAGE_START_CAPTURE)].rep_id: {
        ...
        // TODO: Implement camera trigger start logic
        // This should interface with AP_Camera to start interval-based capture
        start_capture_response.success = AP_DDS_External_Control::start_image_capture(start_capture_request.camera_id, start_capture_request.interval_seconds, start_capture_request.total_images);
        STRCPY(start_capture_response.message, "Image capture started successfully");
        start_capture_response.sequence_start = AP_DDS_External_Control::image_current_seq(start_capture_request.camera_id) + 1; // TODO: Get actual sequence number
    ...
    case services[to_underlying(ServiceIndex::IMAGE_STOP_CAPTURE)].rep_id: {
        ...
        stop_capture_response.total_captured = AP_DDS_External_Control::total_image_cap(stop_capture_request.camera_id); // TODO: Get actual total captured
...
void AP_DDS_Client::update()
{
    ...
    if (cur_time_ms - last_camera_feedback_time_ms > DELAY_CAMERA_FEEDBACK_TOPIC_MS) {
        if (update_topic(camera_feedback_topic, active_camera_instance)) {
...
#if AP_DDS_CAMERA_CAPTURE_STATUS_PUB_ENABLED
    if (cur_time_ms - last_camera_capture_status_time_ms > AP_DDS_DELAY_CAMERA_CAPTURE_STATUS_TOPIC_MS) {
        if (update_topic(camera_capture_status_topic, active_camera_instance)) {
