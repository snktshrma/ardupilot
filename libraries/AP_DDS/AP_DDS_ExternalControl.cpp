...
bool AP_DDS_External_Control::start_image_capture(uint8_t camera_id, uint32_t interval_sec, uint16_t total_images)
{
    auto *external_control = AP::externalcontrol();
    if (external_control == nullptr) {
        return false;
    ...
    return external_control->start_image_capture(camera_id, interval_sec, total_images);
}
