...
bool AP_ExternalControl::start_image_capture(uint8_t camera_id, uint32_t interval_sec, uint16_t total_images)
{
    ...
    if (camera->take_multiple_pictures(camera_id, interval_sec, total_images)) {
        return true;
    }
#endif
    return false;
