...
void NavEKF3_core::SelectFlowFusion()
{
    ...
    gndOffsetValid = ((imuSampleTime_ms - gndHgtValidTime_ms) < 5000) || (activeHgtSource == AP_NavEKF_Source::SourceZ::RANGEFINDER) || ((ofDataDelayed.ground_distance > 0.1f) && (ofDataDelayed.ground_distance < 100.0f));
    // Perform tilt check
...
void NavEKF3_core::EstimateTerrainOffset(const of_elements &ofDataDelayed)
{
    ...
    bool fuseGroundDist = ((ofDataDelayed.ground_distance > 0.1) && (ofDataDelayed.ground_distance < 100)) ;
    ...
    if ((!rangeDataToFuse && cantFuseFlowData) || (!fuseGroundDist && cantFuseFlowData) || (activeHgtSource == AP_NavEKF_Source::SourceZ::RANGEFINDER)) {
        ...
        if (fuseGroundDist) {
            GCS_SEND_TEXT(MAV_SEVERITY_INFO, "Ground Distance%f -------------------------------------------------------------------",ofDataDelayed.ground_distance);
